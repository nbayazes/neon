...
public:
    ...
    D3D12_SHADER_RESOURCE_VIEW_DESC GetShaderViewDesc() const {
        ...
        if (metadata.IsCubemap()) {
            ...
            desc.Texture2DArray.MipLevels = (uint)metadata.mipLevels;
            desc.Texture2DArray.ArraySize = (uint)metadata.arraySize;
            return desc;
        }
        else {
            // 2D texture
    ...
    // Loads an array of mipmapped textures
    template <class TData>
    bool LoadArray2D(List<List<span<TData>>> slices, size_t width, size_t height, DXGI_FORMAT format = DXGI_FORMAT_R8G8B8A8_UNORM_SRGB) {
        if (slices.empty()) return false;

        if (FAILED(Initialize2D(format, width, height, slices.size(), slices.front().size())))
            return false;

        for (int s = 0; s < slices.size(); ++s) {
            auto& slice = slices[s];
            // Fill each level
            for (size_t mip = 0; mip < slice.size(); ++mip) {
                const auto img = GetImage(mip, s, 0);
                memcpy(img->pixels, slice[mip].data(), img->slicePitch);
            }
        }
