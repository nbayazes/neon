...
public:
    ...
    const char* what() const override {
        ...
        (void)sprintf_s(s_str, "Failure with HRESULT of %08X", static_cast<unsigned int>(result));
        return s_str;
    }
...
inline void DebugTrace(_In_z_ _Printf_format_string_ const char* format, ...) noexcept {
    ...
    char buff[1024] = {};
    (void)vsprintf_s(buff, format, args);
...
struct virtual_deleter {
    void operator()(void* p) const noexcept { if (p) VirtualFree(p, 0, MEM_RELEASE); }
};
#endif
