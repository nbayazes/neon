...
namespace ImGui {
//bool ToggleButton(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg, float borderSize) {
...
struct FrameResources {
    UploadBuffer<shaders::imgui::Vertex> VertexBuffer = { 20000, "imgui vertices" };
    UploadBuffer<uint16> IndexBuffer = { 40000, "imgui indices" };
};
...
void Initialize(SDL_Window* window, float fontSize) {
    ...
    //ImGui_ImplWin32_Init(shell::hwnd);
...
void SetRenderState(const ImDrawData* drawData, ID3D12GraphicsCommandList* cmdList, const FrameResources* fr) {
    ...
    auto proj = DirectX::SimpleMath::Matrix::CreateOrthographicOffCenter(L, R, B, T, 0.0, -2.0f);

    // Setup viewport
    CD3DX12_VIEWPORT vp(0.0f, 0.0f, drawData->DisplaySize.x * drawData->FramebufferScale.x, drawData->DisplaySize.y * drawData->FramebufferScale.y);
    ...
    auto context = neon::gfx::GetGraphicsContext();
...
void RenderDrawData(const ImDrawData* drawData, GraphicsContext* context) {
    if (!drawData || !context) return;
    ...
    for (int n = 0; n < drawData->CmdListsCount; n++) {
        ...
        fr->VertexBuffer.Copy({ (shaders::imgui::Vertex*)drawList->VtxBuffer.Data, (size_t)drawList->VtxBuffer.Size });
        fr->IndexBuffer.Copy({ drawList->IdxBuffer.Data, (size_t)drawList->IdxBuffer.Size });
    }
...
void Draw() {
    RenderDrawData(ImGui::GetDrawData(), neon::gfx::GetGraphicsContext());
}
